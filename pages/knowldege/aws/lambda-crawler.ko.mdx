...
제가 크롤링 하고자 하는 대상이 간단했고 이후에 AWS Lambda에 올릴 것을 염두했기 때문입니다.
...
(크롤러 코드가 길어 [GitHub 코드 링크](https://github.com/0-Chan/news-headline-crawler)에서 확인하실 수 있습니다.)
...
전체적인 순서는 이렇습니다.
1. 작성된 크롤러를 핸들러에 통합
2. `npm install`로 필요한 모듈 설치
3. `unzip -r ../myFunc.zip ./*`로 zip 파일로 압축 (node_modules를 포함)
4. aws-cli 또는 AWS Console로 zip 파일 업로드

이렇게 하여 AWS Lambda에서 원하는 모듈을 쓸 수 있습니다.

만약 업로드 할 zip 파일의 용량이 50mb가 넘는다면 S3을 이용하여 올릴 수 있습니다.  
(참고: [AWS Lambda 할당량](https://docs.aws.amazon.com/ko_kr/lambda/latest/dg/gettingstarted-limits.html))
### ⅱ. API Gateway 연결
...
(https://gdrnf2bnbd.execute-api.ap-northeast-2.amazonaws.com/default/naver-headline-crawler)  
...
`$ curl -X GET -H "x-api-key: 키값" -H "Content-Type: application/json" -d '{"key":"val"}' https://1234567890.execute-api.ap-northeast-2.amazonaws.com/default/naver-headline-crawler`

## 결과물
![crawled-news](../../../public/img/aws/crawled-news.webp)

사실 AWS Lambda를 다뤄보면서 여러 시행착오를 겪었습니다.  
하지만 AWS 공식 문서가 자세히 적혀있어 차근차근 시도해보았고,  
직접 User를 생성하고 IAM로 권한 부여 등을 해보면서 조금씩 익숙해 질 수 있었습니다.
